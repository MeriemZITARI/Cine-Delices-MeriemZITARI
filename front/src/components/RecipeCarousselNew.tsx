...
const RecipeCarouselNew: React.FC<RecipeCarouselProps> = ({ recipes }) => {
  ...
  const NextArrow = (props: { onClick?: () => void }) => {
    ...
    return (
      <div
        className="absolute right-4 top-1/2 -translate-y-1/2 z-10 cursor-pointer bg-white/80 text-red-500 p-2 rounded-full shadow-lg hover:bg-white hover:text-red-600 transition-all"
        onClick={onClick}
      >
  ...
  const PrevArrow = (props: { onClick?: () => void }) => {
    ...
    return (
      <div
        className="absolute left-4 top-1/2 -translate-y-1/2 z-10 cursor-pointer bg-white/80 text-red-500 p-2 rounded-full shadow-lg hover:bg-white hover:text-red-600 transition-all"
        onClick={onClick}
      >
  ...
  const handleMovieClick = (e: React.MouseEvent<HTMLDivElement>, movieId: string) => {
    e.preventDefault();
    e.stopPropagation();
              ...
              <div className="mt-2 text-center">
                ...
                {recipe.movie && (
                  <p className="text-xs text-gray-500 mt-1">
                    Inspiré par: {recipe.movie.title} ({new Date(recipe.movie.releaseDate).getFullYear()})
                  </p>
                )}
              </div>
            </Link>
...
export default RecipeCarouselNew;
